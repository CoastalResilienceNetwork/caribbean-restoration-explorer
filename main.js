...
define([
       ...
       function (declare,
			...
			return declare(PluginBase, {
				//Plugin Settings
				toolbarName: "Caribbean Restoration Explorer", //toolbarName is used as the title in the Identify Window
				toolbarType: "sidebar",
				fullName: _hoverText,
				...
				*/
				initialize: function (frameworkParameters) {
					...
					this.textnode = domConstruct.create("div", { innerHTML: "<p style='padding:10px 0px 0px 10px;'>" + this.explorerObject.text + "</p>" });
        ...
        rebuildOptions: function() {
					...
					$('<h3 style="margin-top: 10px; padding-left:10px;">' + name + '</h3>').appendTo($(this.ddNode));
					var display = (this.usableRegions.length > 1) ? "block" : "none";
					outerBox = $('<div class="eeheader" style="display: ' + display + '; padding-left:10px;" />').appendTo($(this.ddNode));
				...
				changeGeography: function(geography, zoomto) {
					...
					array.forEach(geography.tabs, lang.hitch(this,function(tab, t){
						...
						array.forEach(tab.items, lang.hitch(this,function(entry, i){
							...
							if (entry.type == "text") {
								nslidernodeheader = domConstruct.create("div", {
									style:"margin-top:10px;margin-bottom:20px;line-height:1;", 
					...
					if (geography.ancillaryUrl != undefined) {
						...
						this.ancillaryLayer.setImageFormat("png32");
						slayers = new Array();
				...
				*/
				addAncillary: function(b,ancillary, compText) {
					...
					this.ancillaryLayer2.setImageFormat("png32");
				...
				*/
				updateService: function(zoomto) {
					...
					if (this.isVector == true)  {
						...
						legIndexes = [1,2,3];
					    if (zoomto == true) {
							if (this.geography.hasOwnProperty('initialExtent') && this.geography.initialExtent.length == 4){
								
								var xmin,ymin,xmax,ymax;
								xmin = this.geography.initialExtent[0];
								ymin = this.geography.initialExtent[1];
								xmax = this.geography.initialExtent[2];
								ymax = this.geography.initialExtent[3];
								var sr = this.map.spatialReference;
								var newextent = new Extent(xmin,ymin,xmax,ymax, sr);
								this.map.setExtent(newextent, true);
					...
					if (selectedIndex == -1) {
						...
						//toggle visibility of tab[0] main content
						if(dojoquery(tabs[0].containerNode).parent()[0]) {
